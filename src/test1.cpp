...
public:
    ...
    static void warmup()
    {
        ...
        {
            deallocate(ptr, size);  // 使用实际分配的大小进行释放
        }
    ...
    static void testSmallAllocation()
    {
        ...
        // 测试内存池
        {
            ...
            {
                ptrs.push_back(allocate(SMALL_SIZE));

                // 模拟真实使用：部分立即释放
                if (i % 4 == 0)
                {
                    //std::cout << i << ' ';
                    deallocate(ptrs.back(), SMALL_SIZE);
                    ptrs.pop_back();
                }
            }
    ...
    static void testMultiThreaded()
    {
        ...
        auto threadFunc = [](bool useMemPool)
        {
            ...
            {
                size_t size = dis(gen);
                void* ptr = useMemPool ? allocate(size)
                                     : new char[size];
                ...
                {  // 75%的概率释放
                    ...
                    if (useMemPool) {
                        deallocate(ptrs[index].first, ptrs[index].second);
                    } else {
        ...
        // 测试内存池
        {
            ...
            for (size_t i = 0; i < NUM_ALLOCS; ++i)
            {
                ...
                if (i % 100 == 0 && !ptrs.empty())
                {
                    ...
                    {
                        deallocate(ptrs.back().first, ptrs.back().second);
                        ptrs.pop_back();
                    }
                }
            }
