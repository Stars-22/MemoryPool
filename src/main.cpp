...
int main() {
    ...
    // 1. 调用函数
    cc.requestPageToPageCache(1080);
    // assert(page != nullptr);
    // std::cout << "requestPageToPageCache returned non-null\n";

    // 2. 检查每个 bucket 是否初始化
    for (int i = 0; i < 10; i++) {
        assert(cc.buckets[i].mp != nullptr);
        std::cout << "Bucket " << i << " initialized with slot size = "
                  << cc.buckets[i].mp << "\n";
    }
    ...
    for (int i = 0; i < 10; i++) {
        std::cout << i << std::endl;
        auto head = cc.buckets[i].mp;
        assert(head->nextPool != nullptr);
        assert(head->prevPool != nullptr);
        assert(head->nextPool->prevPool == head);
        assert(head->prevPool->nextPool == head);
    }
    std::cout << "Linked list check passed\n";

    // 4. 多次调用
    cc.requestPageToPageCache(1080);
    // assert(page2 != nullptr);
    // std::cout << "Second call works too\n";
    auto mp = cc.allocate(8);
    std::cout << mp->getSlotSize() <<std::fflush;
